...
# (nav label, output filename) — only the two most representative views.
PAGES = [
...
**Files:**
- Create (generated): `docs/images/overview.png`, `docs/images/ask-the-data.png`
...
```
Expected: `captured overview.png` and `captured ask-the-data.png`.

- [ ] **Step 4: Stop the app and confirm both PNGs exist and are non-trivial**
...
./venv/bin/python -c "
import os
for f in ['overview','ask-the-data']:
...
"
```
Expected: both PNGs > 10KB. (If the Ask-the-Data capture is flaky, fall back to Overview-only and drop its README reference in Task 12.)
...
"
```
Expected: `README OK: 2 screenshots referenced and present`
